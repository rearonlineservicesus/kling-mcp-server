...
function createKlingToken() {
    ...
    return jwt.sign({ iss: ACCESS_KEY, exp: now + 1800, nbf: now - 5 }, SECRET_KEY, { algorithm: "HS256", header: { alg: "HS256", typ: "JWT" } });
}
...
function buildServer() {
    ...
    server.tool("kling_get_video_task", "Get Kling AI task status.", {
        ...
        const result = await klingRequest(`/v1/videos/${task_type}/${task_id}`, { method: "GET" });
        return {
            content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
    });
