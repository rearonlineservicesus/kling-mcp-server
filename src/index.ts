...
function buildServer(): McpServer {
  ...
  server.tool(
    "kling_text_to_video",
    "Generate a Kling AI video from text. Defaults to the highest-quality model (kling-v2-6) in pro mode. Override only if user explicitly requests a smaller/faster model.",
    {
      prompt: z.string(),
      ...
      model_name: z
        ...
        .default("kling-v2-6")
        .describe(
          "Kling model version. Default kling-v2-6 (best quality, newest). Use kling-v2-5-turbo for faster output. Use older versions only if the user explicitly requests them."
        ),
      mode: z
        .enum(["std", "pro"])
        .default("pro")
        .describe(
          "Quality mode. 'pro' = higher quality (default). 'std' = faster/cheaper. Always use pro unless the user explicitly asks for speed."
        ),
  ...
  server.tool(
    ...
    "Generate a Kling AI video from an image. Defaults to highest-quality model (kling-v2-6) in pro mode.",
    {
      image: z.string(),
      ...
      model_name: z
        ...
        .default("kling-v2-6")
        .describe(
          "Kling model version. Default kling-v2-6 (best). Use older versions only on explicit user request."
        ),
      mode: z
        .enum(["std", "pro"])
        .default("pro")
        .describe(
          "Quality mode. 'pro' (default) = higher quality. 'std' = faster/cheaper. Use pro unless user asks for speed."
        ),
      duration: z.enum(["5", "10"]).default("5"),
    },
    async (args) => {
